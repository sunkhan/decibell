...
| 11 | ~~`UDP_MAX_PAYLOAD` is 1200 in Rust but 1400 in the C++ header; recv buffer truncates~~ **fixed 2026-08-28** (header → 1200 for video/FEC, `AUDIO_MAX_PAYLOAD` 1400 for audio) | `native/media/video_packet.rs:19` vs `src/common/udp_packet.hpp:14` | **[corroborated]** Any 1400-chunking sender → permanently corrupt-but-"reassembled" video, invisible server-side |
