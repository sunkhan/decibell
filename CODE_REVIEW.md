...
| 11 | ~~`UDP_MAX_PAYLOAD` is 1200 in Rust but 1400 in the C++ header; recv buffer truncates~~ **fixed 2026-08-28** (header → 1200 for every type; client audio cap 1400 → 1200 with the Opus buffer at 1199 and a 320 kbps bitrate clamp on both ends) | `native/media/video_packet.rs:19` vs `src/common/udp_packet.hpp:14` | **[corroborated]** Any 1400-chunking sender → permanently corrupt-but-"reassembled" video, invisible server-side |
