// Time-series ring buffer for the user-panel connection telemetry
// popover. Backend emits a sample every ~2s while connected to a
// voice channel (see VoiceEvent::ConnectionStats in pipeline.rs). We
// keep up to 5 minutes of history so the popover can plot the recent
// ping + packet loss curves Discord-style.
...
// If samples stop arriving for this long, drop history on the next
// push so the graph doesn't draw a long horizontal segment between
// two unrelated voice sessions.
