...
export interface ActiveAudio {
  ...
  serverId: string;
  channelId: string;
  /// `decibell-attachment://attach/<serverId>/<attachmentId>` URL.
  /// Tauri passed a tempPath for native cleanup; we don't need that
  /// because Chromium handles HTTP caching for the custom protocol.
  src: string;
  filename: string;
}
