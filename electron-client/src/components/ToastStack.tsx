...
function ToastCard({ toast }: { toast: ToastT }) {
  ...
  return (
    <div
      className={`pointer-events-auto flex w-full max-w-md items-start gap-3 rounded-lg border bg-bg-light px-4 py-3 shadow-float animate-[toastIn_0.25s_ease_both] ${SEVERITY_BORDER[toast.severity]}`}
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {toast.body && (
          <p className="mt-0.5 text-[11px] leading-[1.55] text-text-muted [overflow-wrap:anywhere]">
            {toast.body}
          </p>
        )}
      </div>
      <button
        ...
        aria-label="Dismiss"
        className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-sm text-text-muted transition-colors hover:bg-surface-hover hover:text-text-secondary"
      >
