...
// host registers.
//
// PR8 Electron adaptations vs the tauri-era original:
...
//     capture works without forcing a CORS roundtrip.
...
function PersistentPlayer({ active, hostElement }: ActivePlayerProps) {
  ...
  // Desired playback position. Initialised once from the cached
  // lastTime so re-clicking an already-watched video resumes at
  // "Paused at 0:06" instead of restarting. Read by the initial seek
  // in useLayoutEffect below and by the error-recovery handler.
  // Critically: nothing else writes to this ref. In particular, the
  // timeupdate handler must NOT mirror v.currentTime here — Chromium
  // emits a synthetic timeupdate at currentTime=0 when v.src is set,
  // and that 0 would clobber the resume target before loadedmetadata
  // fires.
  ...
  useLayoutEffect(() => {
    ...
    desiredTimeRef.current = cached?.lastTime ?? 0;
    v.src = active.src;

    // Attach the seek-then-play handler synchronously, right after
    // setting src — useEffect's microtask gap is enough for Chromium
    // to fire loadedmetadata on a cached resource, which would
    // bypass a listener attached later. We only need it once: the
    // initial seek lands at loadedmetadata and the user owns the
    // playhead from then on (scrubbing, keyboard arrows, etc).
    const startPlayback = () => {
      const t = desiredTimeRef.current;
      if (t > 0 && isFinite(v.duration) && v.duration > 0 && t < v.duration) {
        v.currentTime = t;
      }
      v.play().catch(() => {});
    };
  ...
  useEffect(() => {
    if (!hostElement) {
      // Intentionally keep the last `pos` value. If we cleared it,
      // the wrapper would fall back to the 1×1 offscreen park style,
      // and Chromium's media pipeline treats <video> elements that
      // suddenly become 1×1 (or visually-zero) as backgrounded —
      // which can cause the decoder to throttle or error out
      // ("Unsupported pixel format: -1") and leave the element
      // stuck on its last frame when the host re-mounts. Holding
      // the previous geometry while only toggling visibility avoids
  ...
  const setFullscreen = async (on: boolean) => {
    ...
    try {
      await window.decibell.window.setFullscreen(on);
  ...
  useEffect(() => {
    return () => {
      if (ownsWindowFullscreen.current) {
        window.decibell.window.setFullscreen(false).catch(() => {});
      }
    };
  }, []);
  ...
  useEffect(() => {
    return () => {
      ...
      try {
        ...
        // Tainted canvas (CORS) or other capture failure — fall back
        // to whatever poster (server thumbnail or none) was already in
        // place for this attachment.
      }
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  useEffect(() => {
    ...
    const onTime = () => {
      ...
      // NOTE: do NOT overwrite desiredTimeRef here. Setting v.src
      // emits a synthetic timeupdate at currentTime=0 *before*
      // loadedmetadata fires — and that 0 would clobber the resume
      // target we captured from the cache at mount, making the seek
      // a no-op. desiredTimeRef is a one-shot resume target, not a
      // running playback position; the cache below is the durable
    ...
    // Recover from a real <video> error event (MEDIA_ERR_*). We avoid
    // catching the noisy "Unsupported pixel format: -1" log line —
    // that's a Chromium pixel-format probe failure during decoder
    // negotiation, NOT an actual playback error, so it never reaches
    // the error event. desiredTimeRef holds the canonical playback
    // position (initial cached time, then the latest timeupdate), so
    ...
    const onError = () => {
      ...
      const t = v.currentTime > 0 ? v.currentTime : desiredTimeRef.current;
  ...
  // toggle fullscreen instead. Mirrors YouTube's behaviour.
  ...
  const handleContextMenu = (e: React.MouseEvent) => {
    // Suppress on the controls bar so right-clicking a button doesn't
    // pop up "Save video" over it. Anything outside the bar gets the menu.
  ...
  // (Chromium's scroll-jank avoidance), so calling preventDefault
  // inside an onWheel handler logs "Unable to preventDefault inside
  // passive event listener invocation." every tick. Bind imperatively
  // with {passive:false} instead — same pattern AudioItem already uses.
  ...
  // since the wrapper takes over the viewport, and skipped while the
  // host is gone (we hide the wrapper entirely via opacity below).
  let clipPath: string | undefined;
  if (!fullscreen && hostElement && pos && clipRect) {
  ...
  // visually-zero, which left the decoder in a broken state on
  // re-mount. Holding geometry + only toggling visibility keeps the
  // pipeline happy.
  const hidden = !fullscreen && !hostElement;
  const wrapperStyle: React.CSSProperties = fullscreen
    ...
    : pos
      ...
      : {
          // Only hit on the very first render before any host has
          // ever registered. Park near the corner with a non-trivial
          // size so Chromium treats us as a real video element.
          position: "fixed",
          left: 0,
          top: 0,
          width: 320,
  ...
  return (
    ...
    >
      <video
        ref={videoRef}
        // No `autoPlay` — the dedicated seek-and-play effect below kicks
        // playback off explicitly *after* setting currentTime to the
