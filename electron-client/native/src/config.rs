//! Persisted configuration: AES-256-GCM-encrypted login credentials and
//! plain-JSON `AppSettings`. Lives at `<userData>/config.json`. The
//! encryption key is derived from `hostname() + USER` — purely a
//! local-disk-theft mitigation, not real cryptographic protection.
//!
//! Replaces `tauri::AppHandle::path().app_data_dir()` with
//! `state::boot().user_data_dir`, set once from Electron main's
...
pub struct AppSettings {
    ...
    /// Codec preference toggles. When false, that codec is removed
    /// from the encode list before advertisement to peers, so the
    /// streamer never auto-picks it. Both default true.
...
pub fn clear_credentials() -> Result<(), String> {
    ...
    let mut config: ConfigFile = serde_json::from_str(&data).unwrap_or_default();
