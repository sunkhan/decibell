...
// --- P2P DM calls ---
/// A CallSignal relayed by central (or generated by it: PEER_OFFLINE /
/// NOT_ALLOWED). `from` is always the peer, `to` is always us.
pub const CALL_SIGNAL: &str = "call_signal";
/// Hole punch succeeded on both sockets and the P2P VoiceEngine is up.
pub const CALL_CONNECTED: &str = "call_connected";
/// STUN / bind / punch failed — no media session was started.
pub const CALL_FAILED: &str = "call_failed";
/// An active call lost its peer (no authenticated datagram for 15 s).
...
// ── P2P DM call payloads ─────────────────────────────────────────

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct CallCandidatePayload {
    /// "VOICE" | "MEDIA" — proto CallCandidate.Socket name.
    pub socket: String,
    /// "HOST" | "SRFLX" — proto CallCandidate.Kind name.
    pub kind: String,
    pub ip: String,
    pub port: u32,
}
...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct CallStreamMetaPayload {
    pub codec: i32,
    pub width: u32,
    pub height: u32,
    pub fps: u32,
...
/// Mirror of proto `CallSignal` as the renderer sees it. `kind` is the
/// proto enum name ("INVITE", "ACCEPT", …) so the renderer's state
/// machine and `send_call_signal` speak the same vocabulary.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct CallSignalPayload {
    ...
    pub from: String,
    pub to: String,
    /// base64 of the 32-byte X25519 public key (INVITE / ACCEPT).
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct CallFailedPayload {
    pub call_id: String,
    /// "no_path" | "stun" | "bind" | "aborted"
    pub reason: String,
    pub detail: String,
}
...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct CallDroppedPayload {
    pub call_id: String,
    pub reason: String,
}

pub fn emit_call_signal(p: CallSignalPayload) { send(CALL_SIGNAL, p); }
pub fn emit_call_connected(p: CallConnectedPayload) { send(CALL_CONNECTED, p); }
pub fn emit_call_failed(p: CallFailedPayload) { send(CALL_FAILED, p); }
pub fn emit_call_dropped(p: CallDroppedPayload) { send(CALL_DROPPED, p); }
