...
fn run_encode_loop(
    ...
    while !stop.load(Ordering::Relaxed) {
        ...
        if let Err(e) = encoder.send_bgra(&bgra) {
        ...
        let _ = encoder.for_each_packet(|data, is_key, _pts| {
            ...
            // stream Buffer TSFN keyed by local username.
            events::send_stream_frame(events::StreamFrame {
                username: cfg.local_username.clone(),
                codec: cfg.codec_wire_byte,
                keyframe: is_key,
                timestamp: 0, // pts is encoder-internal; renderer doesn't use this for self-preview
                data: data.to_vec(),
                description: None,
            });
            frames_sent += 1;
