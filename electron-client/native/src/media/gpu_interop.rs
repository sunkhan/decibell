...
//!
//! Revived from the Tauri client for the native Linux stream pipeline
//! (see docs/superpowers/specs/2026-05-15-linux-native-ffmpeg-encoder-
//! design.md). All entry points dlopen their libs (libcuda, libEGL,
//! libGL) so a machine missing them returns `None` from `new()` and the
//! pipeline degrades to the CPU-readback / software encode path.
#![cfg(target_os = "linux")]
