...
//!
//! Revived from the Tauri client (see docs/superpowers/specs/2026-05-15-
//! linux-native-ffmpeg-encoder-design.md).
#![cfg(target_os = "linux")]
...
fn capture_loop(
    ...
    // zwlr_screencopy capture_output's first arg is overlay_cursor (1 =
    // composite the cursor into the frame, 0 = omit).
    let overlay_cursor: i32 = if config.include_cursor { 1 } else { 0 };
    ...
    loop {
        ...
        if in_flight.is_none() {
            let f = mgr.capture_output(overlay_cursor, &output, &qh, ());
        ...
        let next_frame = mgr.capture_output(overlay_cursor, &output, &qh, ());
