...
impl VideoProcessor {
    ...
    /// The destination is typically an NV12 texture handed out by the
    /// FFmpeg encoder's hw_frames_ctx pool, which means it changes per
    /// frame — we must create per-frame input/output views.
    pub fn blit_into(
        &self,
        ...
        unsafe {
            ...
            // Per-frame output view — pool hands out a different NV12 texture
            // each call.
            ...
            let output_view = output_view
                .ok_or("CreateVideoProcessorOutputView returned None")?;

            // pInputSurface is wrapped in ManuallyDrop<Option<T>> in
            // windows-rs 0.61 because the C struct doesn't drop COM refs
            // for us; we do it manually below.
