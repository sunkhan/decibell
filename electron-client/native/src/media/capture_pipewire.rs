//! PipeWire + XDG ScreenCast portal video capture, revived from the Tauri
//! client for the native Linux stream pipeline (see docs/superpowers/
//! specs/2026-05-15-linux-native-ffmpeg-encoder-design.md). Negotiates
//! DMA-BUF buffers (zero-copy → gpu_interop) where the compositor offers
//! them, falling back to SHM `RawFrame`s otherwise.
#![cfg(target_os = "linux")]
...
pub async fn start_capture(
    ...
    let result = tokio::task::spawn_blocking(move || -> Result<_, String> {
        ...
        let (pw_fd, node_id, dbus_conn) = portal_screencast_session(config.include_cursor)?;
...
fn portal_screencast_session(
    include_cursor: bool,
) -> Result<(OwnedFd, u32, zbus::blocking::Connection), String> {
    ...
    let (code, _) = portal_request(
        ...
        &(
            ...
            HashMap::from([
                ...
                // cursor_mode picks how the cursor appears: 1 = HIDDEN,
                // 2 = EMBEDDED (drawn into the frames). Driven by the
                // user's "include cursor" toggle. (4 = METADATA, separate
                // channel — not used; we want it baked into the video.)
                (
                    "cursor_mode",
...
fn pipewire_capture_loop(
    ...
    // Format-pod order = producer preference. SHM-first (default) is the
    // known-safe path. Setting DECIBELL_TRY_DMABUF=1 offers DMA-BUF first
    // to probe what this compositor/driver negotiates — the log line
    // "Negotiated format: ... dmabuf=<bool> modifier=0x<...>" then tells us
    // whether KDE+NVIDIA hands out a LINEAR (mmappable; green light for
    // CUDA zero-copy) or tiled DMA-BUF (the NVIDIA+Wayland+portal
    // limitation — process() will bail with a FATAL note). Reversible:
    // unset the env var to return to SHM.
    let prefer_dmabuf = std::env::var_os("DECIBELL_TRY_DMABUF").is_some();
    if prefer_dmabuf {
        log::info!("[capture] DECIBELL_TRY_DMABUF set — offering DMA-BUF before SHM");
    }
    let mut params = if prefer_dmabuf {
        [
